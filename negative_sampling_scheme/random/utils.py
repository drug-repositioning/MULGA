...
def get_negative_samples(mask,A_orig_arr,topk):
    train_sample_num = np.sum(mask)
    neg_num = topk * train_sample_num 
    unknown_indices = np.where(A_orig_arr == 0)[0]
    sampling_index = np.random.randint(0,len(unknown_indices),neg_num)
    neg_samples = unknown_indices[sampling_index]
    neg_sample_list = np.zeros_like(A_orig_arr)
    neg_sample_list[neg_samples] = 1 
    neg_mask = neg_sample_list.reshape((mask.shape[0],mask.shape[1]))
    return neg_mask
