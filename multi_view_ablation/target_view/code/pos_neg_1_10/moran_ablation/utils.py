...
def Construct_G(A_train_mat,SR,SD):
    SR_ = torch.FloatTensor(SR)
    SD_ = torch.FloatTensor(SD)
    SR_adj = torch.sigmoid(SR_)
    SD_adj = torch.sigmoid(SD_)
    SR_adj_np = SR_adj.numpy()  
    SD_adj_np = SD_adj.numpy()  
    SR_adj_np[SR_adj_np>=0.85] = 1 
    SR_adj_np[SR_adj_np<0.85] = 0   
    SD_adj_np[SD_adj_np>=0.85] = 1 
    SD_adj_np[SD_adj_np<0.85] = 0
    print('SD_adj_np:',np.where(SD_adj_np==0))
    print('SR_adj_np:',np.where(SR_adj_np==0))
    A_row1 = np.hstack((SR_adj_np,A_train_mat))
    A_row2 = np.hstack((A_train_mat.T,SD_adj_np))
