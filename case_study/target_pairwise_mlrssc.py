...
def parse_args():    
    ...
    parser.add_argument('-mu', '--mu_topofallfeature', type=float, nargs='?', default=4,help='Performing MLRSSC.')
    parser.add_argument('-beta1', '--beta1_topofallfeature', type=float, nargs='?', default=0.1,help='Performing MLRSSC.')
...
if __name__=="__main__":
    ...
    print('mu:',mu)
    beta1 = args.beta1_topofallfeature    # determining rank 
    print('beta1:',beta1)
    lambdav = 0.3
    beta2 = 1.0-beta1     # determining sparsity
    print('beta2:',beta2)
    mu_max = 1e6
    pho = 1.2
    iter_max = 50
    ...
    root_path = "/home/jiani.ma/dbMVGCNAE/dataset/protein_feature"
    AAC_path = os.path.join(root_path,"AAC.xlsx") 
    CTD_path = os.path.join(root_path,"CTD.xlsx")
    Moran_path = os.path.join(root_path,"Moran.xlsx")
    PAAC_path = os.path.join(root_path,"PAAC.xlsx")
    
    # Drug Features or Target features
    aac = pd.read_excel(AAC_path).values[:,1:]  
    ctd = pd.read_excel(CTD_path).values[:,1:]  
    moran = pd.read_excel(Moran_path).values[:,1:]  
    paac = pd.read_excel(PAAC_path).values[:,1:]  
